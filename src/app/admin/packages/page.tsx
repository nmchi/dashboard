...
import { toast } from "sonner"; // Lưu ý: Server Component không gọi được toast trực tiếp, ta dùng form action wrapper bên dưới
                ...
                {packages.length === 0 ? (
                    ...
                    <TableRow key={pkg.id} className="hover:bg-slate-50">
                        ...
                        <TableCell className="text-right flex justify-end gap-2">
                            {/* Nút Sửa: Truyền isActive để Admin có thể bật/tắt */}
                            <PackageDialog pkg={{
                                ...pkg,
                            ...
                            {/* Nút Xóa: Gọi Server Action deletePackage */}
                            <form action={async () => {
                                'use server'
                                // Gọi hàm xóa, nếu lỗi thì (ở đây) chưa handle toast UI
                                // nhưng console server sẽ log hoặc revalidate sẽ không thay đổi gì nếu lỗi
