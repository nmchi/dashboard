...
export function normalizeMessage(
    ...
    // Chỉ tách khi CẢ HAI bên có >= 2 chữ số (giữ nguyên decimal như 5.8, 0.5)
    normalized = normalized.replace(/(\d{2,})[\.\/-](\d{2,})/g, '$1 $2');
    // Lặp lại để xử lý chuỗi dài như 12.34.56
    while (/(\d{2,})[\.\/-](\d{2,})/.test(normalized)) {
        normalized = normalized.replace(/(\d{2,})[\.\/-](\d{2,})/g, '$1 $2');
    }
    ...
    // Bước 9: Loại bỏ dấu câu còn lại (giữ dấu "." giữa 2 số - decimal point)
    normalized = normalized.replace(/(\d)\.(\d)/g, '$1{DOT}$2');
